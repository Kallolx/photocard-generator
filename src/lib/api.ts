...
export const adminAPI = {
  ...
  updateUserPlan: async (
    ...
    const { data } = await apiClient.put(`/admin/users/${userId}/plan`, {
      plan,
    });
    return data;
  },
...
export const developerAPI = {
  getApiKey: async () => {
    const { data } = await apiClient.get("/external/keys");
    return data;
  },

  generateApiKey: async () => {
    const { data } = await apiClient.post("/external/keys");
    return data;
  },
};
