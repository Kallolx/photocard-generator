          ...
          {steps.map((step, index) => (
            <div key={index} className="relative">
              <div className="bg-white border-2 border-[#d4c4b0] p-8 h-full hover:border-[#8b6834] transition-colors">
                <div className="text-[#c19a6b] mb-6">{step.icon}</div>
                <div className="text-6xl font-lora font-bold text-[#8b6834] mb-4">
                  {step.number}
                </div>
              ...
              {index < steps.length - 1 && (
                <div className="hidden lg:block absolute top-1/2 left-full ml-1.5 transform -translate-y-1/2 z-10 text-[#c19a6b]">
                  <svg
                    className="w-5 h-5"
                    fill="none"
                    stroke="currentColor"
