...
export default function UpgradeModal({ isOpen, onClose, feature, requiredPlan }: UpgradeModalProps) {
  ...
  const [selectedPlan, setSelectedPlan] = useState<'Basic' | 'Premium'>(requiredPlan);
  ...
  const handleUpgrade = async () => {
    setIsUpgrading(true);
    try {
      const response = await fetch('http://localhost:5000/api/user/upgrade-plan', {
        method: 'POST',
        headers: {
  ...
  const allPlans = [
    {
      ...
      name: 'Professional',
      price: 199,
      features: ['15 cards/day', 'All templates', 'Custom cards', 'No watermark'],
    },
    {
      id: 'Premium',
      name: 'Newsroom',
      price: 499,
      features: ['Unlimited cards', 'Batch processing', 'Priority support', 'API access'],
    },
  ];

  // If Premium is required, only show Premium plan
  const plans = requiredPlan === 'Premium' 
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-xl font-lora font-bold text-[#2c2419]">Upgrade Your Plan</h3>
        ...
        {/* Plans Grid */}
        <div className={`grid ${plans.length === 1 ? 'grid-cols-1 max-w-sm mx-auto' : 'grid-cols-2'} gap-3 mb-6`}>
          {plans.map((plan) => {
            ...
            return (
              <button
                key={plan.id}
                onClick={() => !isCurrentPlan && setSelectedPlan(plan.id as 'Basic' | 'Premium')}
              ...
              >
                ...
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <h4 className="text-sm font-lora font-bold text-[#2c2419]">{plan.name}</h4>
                    <div className="flex items-baseline mt-1">
                      <span className="text-2xl font-lora font-bold text-[#8b6834]">৳{plan.price}</span>
                      <span className="text-xs text-[#5d4e37] font-inter ml-1">/mo</span>
                    </div>
                  </div>
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                    isSelected ? 'border-[#8b6834] bg-[#8b6834]' : 'border-[#d4c4b0]'
                  }`}>
                ...
                <ul className="space-y-1">
                  {plan.features.map((feat, idx) => (
                    <li key={idx} className="flex items-center gap-1.5 text-xs text-[#5d4e37] font-inter">
                      <svg className="w-3 h-3 text-[#8b6834] flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div className="space-y-2">
          <button
            ...
            disabled={isUpgrading || user?.plan === selectedPlan}
