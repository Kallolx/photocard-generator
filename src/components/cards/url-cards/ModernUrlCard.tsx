...
export default function ModernUrlCard({
  ...
  const renderDragOverlay = () => {
    ...
    switch (activeId) {
      case 'logo':
        return (
          <div className="bg-white border border-gray-200 p-2 min-w-[60px] min-h-[30px] shadow-lg flex items-center justify-center">
            {data.logo && <img src={getProxiedImageUrl(data.logo)} alt="Logo" className="object-contain max-w-[100px] max-h-8" />}
          </div>
        );
      case 'dateWeek':
        return (
          <div className="text-white px-3 py-1 rounded shadow-lg" style={{ ...getBackgroundStyle(), fontFamily: fontStyles?.week.fontFamily || "Noto Serif Bengali", fontSize: fontStyles?.week.fontSize || "14px", fontWeight: fontStyles?.week.fontWeight || "500", color: fontStyles?.week.color || "#FFFFFF" }}>
            {getWeekday()} | {getCardDate()}
          </div>
        );
      case 'qrCode':
        return qrCodeUrl ? (
    ...
    >
      ...
      <div className="px-6 pt-4 pb-4 relative z-10">
        ...
        {visibilitySettings.showTitle && (
          <h2
 className="text-white text-center leading-tight mb-3 px-2 py-1"
            style={{
