...
function getTextShadow(preset: string, angle: number, textColor: string): string {
  ...
  switch (preset) {
    case "soft": return `${offsetX * 2}px ${offsetY * 2}px 4px rgba(0, 0, 0, 0.3)`;
    case "hard": return `${offsetX * 3}px ${offsetY * 3}px 0px rgba(0, 0, 0, 0.8)`;
    case "glow": return `0 0 10px ${textColor}, 0 0 20px ${textColor}, 0 0 30px ${textColor}`;
...
export default function MinimalUrlCard({
  ...
  const getBackgroundStyle = () => {
    if (background.type === "gradient") {
      return { background: `linear-gradient(135deg, ${background.gradientFrom || background.color}, ${background.gradientTo || background.color})` };
    }
    return { background: background.color };
  };

  const getImageBlendGradient = () => {
    const baseColor = background.type === "gradient" ? background.gradientFrom || background.color : background.color;
      ...
      case "dateWeek":
        ...
        return (
          <div className="inline-flex items-center z-[2] relative">
            <div className="relative px-4 py-1 backdrop-blur-sm" style={{ background: "rgba(0, 0, 0, 0.4)" }}>
              <div className="absolute left-0 right-2 h-[2px] bg-white" style={{ top: "-4px", borderRadius: "2px" }} />
              <div className="absolute left-0 right-2 h-[2px] bg-white" style={{ bottom: "-4px", borderRadius: "2px" }} />
              <div className="font-bold text-left whitespace-nowrap" style={{
        ...
        return (
          <div className="w-12 h-12 bg-gray-100 shadow-lg rounded-full flex items-center justify-center overflow-hidden flex-shrink-0">
            {data.favicon || data.logo ? (
              <img src={getProxiedImageUrl(data.favicon || data.logo)} alt="Favicon" className="w-full h-full object-contain border-2 border-white rounded-full" onError={(e) => { (e.target as HTMLImageElement).src = "/placeholder-logo.png"; }} />
            ) : (
    ...
    <div id={id} className={fullSize ? "w-[448px] max-w-[448px] mx-auto overflow-hidden shadow-xl relative" : "w-full max-w-md mx-auto overflow-hidden shadow-xl relative"} style={getBackgroundStyle()}>
      <div className="relative overflow-hidden">
        ...
        <div className="relative w-full h-[240px] overflow-hidden">
          <div className="w-full h-full" style={{ border: `${frameBorderThickness}px solid ${frameBorderColor}` }}>
            {data.image ? (
          ...
          <div className="absolute left-0 right-0 bottom-0 h-[140px] pointer-events-none z-[1]" style={{ background: getImageBlendGradient() }} />
        ...
        {visibilitySettings.showLogo && (
          <div className="relative -mt-10 z-20 flex justify-center">
            <div className="relative w-[240px] h-[54px]">
              <svg viewBox="0 0 449 100" className="absolute inset-0 w-full h-full" preserveAspectRatio="none" style={{ filter: "drop-shadow(0 4px 6px rgba(0, 0, 0, 0.3))" }}>
                <path d="M448 15H48.186L0 83H403.721L448 15Z" fill="#FFFFFF" />
                <path d="M35.2299 0L0 49H15.6578L43.0588 10.3158H234.866L244 0H35.2299Z" fill="#FFFFFF" />
                <path d="M413.914 100L449 50H433.406L406.118 89.4737H215.096L206 100H413.914Z" fill="#FFFFFF" />
              </svg>
              <div className="absolute inset-0 flex items-center justify-center p-3">
                {data.logo ? (
                  <img src={getProxiedImageUrl(data.logo)} alt="Logo" className="max-w-[120px] max-h-[40px] object-contain" onError={(e) => { (e.target as HTMLImageElement).src = "/placeholder-logo.png"; }} />
                ) : (
        ...
        <div className="px-6 pt-6 pb-4 relative z-10" style={{
          ...
          {visibilitySettings.showTitle && (
            <h2 className="text-white text-center leading-tight mb-4 px-2 py-1" style={{
